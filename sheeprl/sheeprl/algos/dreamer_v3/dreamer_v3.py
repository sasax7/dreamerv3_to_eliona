        ...
        with torch.inference_mode():
            ...
            with timer("Time/env_interaction_time", SumMetric, sync_on_compute=False):
                ...
                if (
                    ...
                    real_actions_list = envs.call("sample")
                    real_actions      = np.stack(real_actions_list, axis=0)  # shape = (num_envs, 1)
                    ...
                    # Pull the “expert” action from each sub‐environment, just like in the warmup:
                    per_env_actions = envs.call("sample")               # returns a tuple of length num_envs
                    real_actions     = np.stack(per_env_actions, axis=0)  # shape = (num_envs, 1)

                    if not is_continuous:
                        # convert each float → discrete one-hot, exactly as before
                        actions = np.concatenate(
                            [
                                F.one_hot(torch.as_tensor(act), act_dim).numpy()
                                for act, act_dim in zip(real_actions.reshape(len(actions_dim), -1), actions_dim)
                            ],
                            axis=-1,
                        )
                    else:
